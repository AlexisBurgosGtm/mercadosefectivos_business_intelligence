...
router.get('/getVentasVendedores', async function(req,res){
    ...
    let qryx = `SELECT  CODVEN, NOMEMPLEADO AS NOMVEN, 
    SUM(TOTALCOSTO) AS TOTALCOSTO, 
    SUM(TOTALPRECIO) AS TOTALPRECIO,
    (SUM(TOTALPRECIO)-SUM(TOTALCOSTO)) AS UTILIDAD,
    CODSUCURSAL
        ...
        GROUP BY CODVEN, NOMEMPLEADO, CODSUCURSAL
        ORDER BY NOMEMPLEADO`;

    let qry = '';
    qry = `SELECT        CODSUCURSAL, CODVEN, NOMEMPLEADO AS NOMVEN, 
    (SELECT SUM(TOTALPRECIO) FROM BI_RPT_GENERAL AS G2 
...
GROUP BY CODSUCURSAL, CODVEN, NOMEMPLEADO
HAVING        (CODSUCURSAL IN(${empresas})) AND (ANIO IN(${anio})) AND (MES IN(${mes}))
